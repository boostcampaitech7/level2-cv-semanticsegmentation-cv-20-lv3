...
def get_prediction(image_path):
    '''
    1. 이미지를 모델에 통과시킨다.
    - 모델의 pt 값은 직접 그 경로를 지정해준다.
    - 모델은 select로 설정할 수 있게 만들어준다.
    2. 예측값을 뽑아낸다.
    '''
    import os

    model_path = config['paths']['model']['pt_loaded_dir']    
    st.selectbox('모델을 선택해 주세요', os.path.listdir(model_path))
...
def main():
    ...
    if dataset_option == 'Train':
        ...
        st.selectbox('모델을 선택해 주세요', os.listdir('/data/ephemeral/home/level2-cv-semanticsegmentation-cv-20-lv3/result'))
